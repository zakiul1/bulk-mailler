<div x-data="{
    ...
    x-on:copy-contacts.window="copyContacts($event.detail.text)" class="px-4 py-6 lg:px-6">
    <div class="flex flex-col gap-6">
        @if ($this->latestImport)
            @php
                $import = $this->latestImport;
                $importTotal = max((int) ($import->total_read ?? 0), 1);
                $importProcessed = (int) ($import->processed_count ?? 0);
                $importPercent = min(100, max(0, (int) round(($importProcessed / $importTotal) * 100)));
            @endphp

            <div class="border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900"
                wire:poll.2s="pollImportJob">
                <div class="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
                    <div>
                        <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Contact Import Progress</h2>
                        <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                            Real-time progress for the latest contact import.
                        </p>
                    </div>

                    <button type="button" wire:click="clearImportJob"
                ...
                <div class="space-y-4 p-4">
                    <div class="grid gap-4 md:grid-cols-3">
                        ...
                        <div>
                            ...
                            <div class="mt-1 text-sm font-medium text-zinc-900 dark:text-white">
                                {{ ucfirst($import->source_type ?? 'file') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="h-2 w-full overflow-hidden border border-zinc-300 dark:border-zinc-700">
                            <div class="h-full bg-emerald-600 dark:bg-emerald-500"
        ...
        @if ($this->latestDeleteJob)
            <div class="border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900"
                wire:poll.2s="pollDeleteJob">
                <div class="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
                    <div>
                        <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Bulk Delete Progress</h2>
                        <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                            Real-time progress for the latest bulk delete.
                        </p>
                    </div>

                    <button type="button" wire:click="clearDeleteJob"
                        class="border border-zinc-900 bg-zinc-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-zinc-800 dark:border-white dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                        Refresh
                    </button>
                </div>
                ...
                <div class="space-y-4 p-4">
                    ...
                    <div>
                        ...
                        <div class="h-2 w-full overflow-hidden border border-zinc-300 dark:border-zinc-700">
                            <div class="h-full bg-red-600 dark:bg-red-500"
                                style="width: {{ $this->latestDeleteJob->progress_percent }}%;"></div>
                        </div>
                    </div>
        ...
        <div
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('bulk-mailer.contacts.create') }}" wire:navigate
        ...
        <div class="grid gap-4 lg:grid-cols-4">
            <div class="border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 lg:col-span-3">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search by email"
            ...
            <div class="border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <select wire:model.live="listFilter"
                    class="w-full border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
