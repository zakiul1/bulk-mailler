<div x-data="{
    ...
    <div class="flex flex-col gap-6">
        @if ($this->latestImport)
            <div class="border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900" wire:poll.3s>
                <div class="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
                    ...
                    <button type="button" wire:click="refreshImport"
                ...
                <div class="space-y-4 p-4">
                    ...
                    <div>
                        ...
                        <div class="h-2 w-full overflow-hidden border border-zinc-300 dark:border-zinc-700">
                            <div class="h-full bg-zinc-900 dark:bg-white"
                                style="width: {{ $this->importProgressPercent }}%;"></div>
                        </div>
                    </div>

                    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Total Read
                            </div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->total_read }}</div>
                        </div>

                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Processed
                            </div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->processed_count }}</div>
                        </div>

                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Valid</div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->valid_count }}</div>
                        </div>

                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Invalid</div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->invalid_count }}</div>
                        </div>

                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Duplicate
                            </div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->duplicate_count }}</div>
                        </div>

                        <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                            <div class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Inserted</div>
                            <div class="mt-1 text-lg font-semibold text-zinc-900 dark:text-white">
                                {{ $this->latestImport->inserted_count }}</div>
                        </div>
                    </div>
        ...
        <div
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('bulk-mailer.contacts.index') }}" wire:navigate
                ...
                <div class="border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <div class="grid gap-6 p-4">
                        <div>
                            ...
                            <select wire:model="bulk_mailer_contact_list_id"
                        ...
                        <div class="border border-zinc-200 dark:border-zinc-700">
                            <div
                                class="flex border-b border-zinc-200 bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-950">
                                <button type="button" wire:click="switchTab('text')" @class([
                                ...
                                <button type="button" wire:click="switchTab('file')" @class([
                                    'px-4 py-3 text-sm font-medium transition-colors',
                            ...
                            <div class="p-4">
                                @if ($activeTab === 'text')
                                    <div>
                                        <label
                                            class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Paste
                                            Emails</label>
                                        <textarea wire:model.defer="emails_text" rows="14"
                                            placeholder="Paste emails here. Supports line break, comma, semicolon, or mixed format."
                                            class="w-full border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white"></textarea>
                                        @error('emails_text')
                                            <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                        @enderror
                                ...
                                @if ($activeTab === 'file')
                                    <div>
                                        <label
                                            class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Upload
                                            TXT or CSV File</label>
                        ...
                        <div
                            class="flex items-center justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-700">
                            <a href="{{ route('bulk-mailer.contacts.index') }}" wire:navigate
                            ...
                            <button type="button" wire:click="save" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove wire:target="save">Start Import</span>
                                <span wire:loading wire:target="save">Importing...</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col gap-6">
                <div class="border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <div class="space-y-3 p-4 text-sm text-zinc-600 dark:text-zinc-300">
                        ...
                        <div>Large files are processed locally in chunks for better performance.</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-cloak x-show="toast.show" x-transition class="fixed bottom-4 right-4 z-[60] w-full max-w-sm">
        <div class="border px-4 py-3 shadow-lg"
