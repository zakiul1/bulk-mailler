<div class="px-4 py-6 lg:px-6">
    <div class="flex flex-col gap-6">
        <div
            class="flex items-start justify-between border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <div>
            ...
            <div class="flex items-center gap-2">
                @if ((int) $campaign->failed_count > 0)
                    <button type="button" wire:click="retryFailed"
                        wire:confirm="Are you sure you want to retry all failed emails for this campaign?"
                        class="border border-amber-300 px-4 py-2 text-sm text-amber-700 hover:bg-amber-50 dark:border-amber-800 dark:text-amber-300 dark:hover:bg-amber-950">
                        Retry Failed Emails
                    </button>
                @endif

                <a href="{{ route('bulk-mailer.campaigns.index') }}" wire:navigate
                            ...
                            @forelse ($recipientLogs as $recipient)
                                <tr>
                                    ...
                                    <td class="px-4 py-4 align-top text-sm">
                                        @php
                                            $statusValue = $recipient->status?->value ?? $recipient->status;
                                        @endphp
                                        ...
                                        <span @class([
                                            ...
                                            'border-amber-300 text-amber-700 dark:border-amber-800 dark:text-amber-300' =>
                                                $statusValue === 'pending',
                                            'border-zinc-300 text-zinc-700 dark:border-zinc-700 dark:text-zinc-300' => !in_array(
                                                $statusValue,
                                                ['sent', 'failed', 'pending'],
                                                true),
                                        ])>
                                    ...
                                    <td
                                        class="px-4 py-4 align-top text-sm text-zinc-900 dark:text-white whitespace-nowrap">
