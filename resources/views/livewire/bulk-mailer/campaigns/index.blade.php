<div class="px-4 py-6 lg:px-6">
    <div class="flex flex-col gap-6">
        <div
            class="flex items-center justify-between border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            <div>
                <h1 class="text-xl font-semibold text-zinc-900 dark:text-white">Campaigns</h1>
                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                    Create campaigns, connect templates, choose categories, assign SMTP groups, test delivery, and
                    launch sending.
                </p>
            </div>
        ...
        <div class="overflow-hidden border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900"
            ...
            @if ($shouldPoll)
                <div
                    class="border-b border-zinc-200 bg-amber-50 px-4 py-2 text-xs text-amber-700 dark:border-zinc-700 dark:bg-amber-950/40 dark:text-amber-300">
                            ...
                            <tr wire:key="campaign-row-{{ $row->id }}">
                                    ...
                                    @if (($row->status?->value ?? $row->status) === 'processing' || ($row->status?->value ?? $row->status) === 'scheduled')
                                        <div class="mt-2">
                                            <div
                                                ...
                                                <span>
                                                    Progress:
                                                    {{ number_format($processedCount) }}/{{ number_format($totalRecipients) }}
                                                </span>
                                                <span>{{ $progressPercent }}%</span>
                                            </div>

                                            <div
                                                class="h-2 w-full overflow-hidden rounded bg-zinc-200 dark:bg-zinc-800">
                                                <div class="h-2 bg-zinc-900 transition-all duration-300 dark:bg-white"
                                ...
                                <td class="px-4 py-4 align-top">
                                    <div class="flex flex-wrap gap-2">
                                        @forelse ($row->lists as $list)
                                            <span
                                                class="border border-zinc-300 px-2 py-1 text-xs text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">
                                                {{ $list->name }}
                                            </span>
                                        @empty
                                ...
                                <td class="px-4 py-4 align-top">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        <a href="{{ route('bulk-mailer.campaigns.show', $row) }}" wire:navigate
    ...
    @if ($showFormModal)
        <div class="fixed inset-0 z-40 overflow-y-auto bg-black/50">
            <div class="flex min-h-full items-start justify-center px-4 py-6">
                <div
                    class="flex max-h-[90vh] w-full max-w-5xl flex-col border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                    <div
                    ...
                    <div class="overflow-y-auto p-6">
                        <div class="grid gap-4">
                            <div>
                                <label class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Campaign
                                    Name</label>
                                <input type="text" wire:model.defer="name" name="campaign_name"
                                    id="campaign_name" autocomplete="off"
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    <label
                                        class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Template</label>
                                    <select wire:model.live="bulk_mailer_template_id" name="campaign_template_id"
                                        id="campaign_template_id" autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                ...
                                <div>
                                    <div class="mb-2 flex items-center justify-between">
                                        <label class="block text-sm font-medium text-zinc-900 dark:text-white">Select
                                            Categories</label>

                                        <a href="{{ route('bulk-mailer.lists.index') }}" wire:navigate
                                    ...
                                    <div class="border border-zinc-300 dark:border-zinc-700">
                                        <div class="border-b border-zinc-200 p-3 dark:border-zinc-700">
                                            <input type="text" wire:model.live.debounce.300ms="listSearch"
                                                name="campaign_list_search" id="campaign_list_search"
                                        ...
                                        <div class="max-h-64 overflow-y-auto">
                                            @forelse ($this->filteredLists as $list)
                                                <label
                                                    class="flex cursor-pointer items-center gap-3 border-b border-zinc-200 px-3 py-2 text-sm last:border-b-0 dark:border-zinc-800">
                                                    <input type="checkbox" wire:model.live="selected_lists"
                                                        value="{{ $list->id }}"
                                                        class="h-4 w-4 border border-zinc-300 dark:border-zinc-700">
                                                    <span
                                                        class="text-zinc-900 dark:text-white">{{ $list->name }}</span>
                                                </label>
                                            @empty
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div>
                                    <label
                                        class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Segment</label>
                                    <select wire:model.live="bulk_mailer_segment_id" name="campaign_segment_id"
                                        id="campaign_segment_id" autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                    ...
                                    @error('bulk_mailer_segment_id')
                                        <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>

                                <div>
                                    <label class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">SMTP
                                        Group</label>
                                    <select wire:model.live="bulk_mailer_smtp_group_id" name="campaign_smtp_group_id"
                                        id="campaign_smtp_group_id" autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                    ...
                                    @error('bulk_mailer_smtp_group_id')
                                        <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    <label
                                        class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Status</label>
                                    <select wire:model.live="status" name="campaign_status" id="campaign_status"
                                        autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                        <option value="draft">Draft</option>
                                        <option value="scheduled">Scheduled</option>
                            ...
                            <div class="border border-zinc-200 p-3 dark:border-zinc-700">
                                <label
                                    ...
                                    <input type="checkbox" wire:model.live="ab_testing_enabled"
                                        class="h-4 w-4 border border-zinc-300 dark:border-zinc-700">
                            ...
                            @if ($ab_testing_enabled)
                                <div class="grid gap-4 md:grid-cols-2">
                                    <div>
                                        <label
                                            class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Subject
                                            A</label>
                                        <input type="text" wire:model.defer="subject_a" name="campaign_subject_a"
                                            id="campaign_subject_a" autocomplete="off"
                                            class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                        @error('subject_a')
                                            <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                        @enderror
                                    ...
                                    <div>
                                        <label
                                            class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Subject
                                            B</label>
                                        <input type="text" wire:model.defer="subject_b" name="campaign_subject_b"
                                            id="campaign_subject_b" autocomplete="off"
                                            class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                        @error('subject_b')
                                            <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                        @enderror
                            ...
                            @else
                                <div>
                                    <label
                                        class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Subject</label>
                                    <input type="text" wire:model.defer="subject" name="campaign_subject"
                                        id="campaign_subject" autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                    @error('subject')
                                        <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <div>
                                    <label
                                        class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">Schedule
                                        Time</label>
                                    <input type="datetime-local" wire:model.defer="scheduled_at"
                                        name="campaign_scheduled_at" id="campaign_scheduled_at" autocomplete="off"
                                        class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                                    @error('scheduled_at')
                                        <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                                    @enderror
                    ...
                    <div
                        class="flex items-center justify-end gap-3 border-t border-zinc-200 px-6 py-4 dark:border-zinc-700">
                        <button type="button" wire:click="closeModals"
    ...
    @if ($showTestModal)
        <div class="fixed inset-0 z-40 overflow-y-auto bg-black/50">
            <div class="flex min-h-full items-center justify-center px-4 py-6">
                <div
                    class="flex max-h-[90vh] w-full max-w-lg flex-col border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <div class="overflow-y-auto p-6">
                        <form autocomplete="off">
                            <input type="email" name="fake_campaign_test_email" autocomplete="email"
                                class="hidden">
                        </form>
                        ...
                        <input type="email" wire:model.defer="test_email"
                            wire:key="campaign-test-email-{{ $testId ?? 'new' }}"
                            name="campaign_test_email_{{ $testId ?? 'new' }}"
                            id="campaign_test_email_{{ $testId ?? 'new' }}" autocomplete="off" autocapitalize="off"
        ...
        <div class="fixed inset-0 z-40 overflow-y-auto bg-black/50">
            <div class="flex min-h-full items-center justify-center px-4 py-6">
                <div
                    ...
                    <div class="overflow-y-auto p-6">
                        <label class="mb-2 block text-sm font-medium text-zinc-900 dark:text-white">New Schedule
                            Time</label>
                        <input type="datetime-local" wire:model.defer="reschedule_at" name="campaign_reschedule_at"
                            id="campaign_reschedule_at" autocomplete="off"
                            class="w-full border border-zinc-300 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                        @error('reschedule_at')
                            <div class="mt-1 text-xs text-red-600">{{ $message }}</div>
                        @enderror
                    ...
                    <div
                        ...
                        <button type="button" wire:click="reschedule"
                            class="border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-800 dark:border-white dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
                            Save
                        </button>
                    </div>
